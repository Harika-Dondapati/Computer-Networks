...
using namespace std;
class CRC{
    string input,divisior,divident,result;
 ...
 string fun_xor(string a, string b)
 {
    ...
    void getdata()
    {
        cout<<"\nEnter Input: ";
        cin>>input;
        cout<<"/n Enter coefficients of generator polynomial:";
        cin>>divisior;

        len_gen = divisior.length();
...
void receiver_side()
{
 ...
 for(int i=0;i<len_gen-1;i++)
    ...
    if(flag==0)
    cout<<"\n correct Data Received Without ant Error";
    else
